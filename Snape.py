...
class SnippetFile:
    ...
    def __init__(self):
        ...
        with open(self.DBFILE) as fobj:
            self.snippets = {
                ...
                for name, data in json.loads(fobj.read()).items()
            }
    ...
    def add(self, name):
        ...
        self.snippets[name] = {"text": text, "lang": lang}
        self.save()

    def search(self, query):
        query = query.lower()
        for key, snippet in self.snippets.items():
            if query in key.lower() or name in snippet.content:
                output_pair(key, db[key])
...
class Snippet:
    ...
    def text(self):
        ...
        return highlight(self.content, lexer, TerminalFormatter())
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        if args in snippet_file:
            ...
            snippet_file.search(db, " ".join(sys.argv[2:]))
        else:
            print(USAGE)
            sys.exit(1)
